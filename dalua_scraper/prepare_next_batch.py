...
def opt(title):
 bits=[]
 # Preserve extended sizes before generic size words so X-Small/X-Large are not collapsed.
 patterns=[
  r'(?i)\b\d+(?:\.\d+)?\s*(?:kg|g|l|ml|cm|mm)\b',
  r'(?i)\(([^)]+)\)',
  r'(?i)\b(x[- ]?small|xs|x[- ]?large|xl|xxl|fine|small|medium|large|black|grey|gray|pale|yellow|red|purple|blue|white)\b'
 ]
 for pat in patterns:
  for m in re.finditer(pat,title):
   v=(m.group(1) if m.lastindex else m.group(0)).strip()
   vl=v.lower().replace('-',' ')
   canonical={'x small':'X-Small','xs':'X-Small','x large':'X-Large','xl':'X-Large','xxl':'XXL'}.get(vl,v)
   if canonical.lower() not in {x.lower() for x in bits}:bits.append(canonical)
...
def main():
 ...
 for family,rs in sorted(groups.items()):
  ...
  options=[{'source_title':r['dalua_title'],'proposed_option':opt(r['dalua_title']),'cost_plus_gst':r['dalua_cost_plus_gst'],'markdown_plus_gst':r['dalua_markdown_plus_gst'],'source_url':r['dalua_product_url'],'image_url':r['dalua_image_url']} for r in rs]
  option_names=[o['proposed_option'].lower() for o in options]
  item={'family':family,'source_rows':len(rs),'proposed_product_title':family.title(),'options_json':json.dumps(options,separators=(',',':'))}
  hay=(family+' '+' '.join(r['dalua_title'] for r in rs)).lower()
  if len(set(option_names)) != len(option_names):
   item['status']='hold_duplicate_option_labels';hold.append(item)
  elif any(t in hay for t in HOLD_TERMS):
   item['status']='hold_non_freshwater_or_ambiguous';hold.append(item)
  else:
   item['status']='safe_for_shopify_duplicate_audit';safe.append(item)
